...
namespace EOL {
    ...
    bool inBoundaryN(const MatrixXd& bounds, const Vector3d& P, const ARCSim::Vec3& e2, const double& brange) {
        int b1, b2, corner = 0;
        for (b1 = 0; b1 < bounds.cols(); b1++) {
            if (b1 == bounds.cols() - 1) {
                b2 = 0;
            ...
            if (linePointDist(bounds.block<3, 1>(0, b1), bounds.block<3, 1>(0, b2), v2e(vert->u)) < brange) {
                ...
                for (int e = 0; e < node->adje.size(); e++) {
                    ...
                    if (edge->preserve) {
                        ...
                        Vector3d B = bounds.block<3, 1>(0, b2);
                        Vector3d AB = A - B;
                        ARCSim::Node* n1 = other_node(edge, node);
    ...
    void addGeometry(ARCSim::Mesh& mesh, const MatrixXd& boundaries, const vector<shared_ptr<btc::Collision> > cls)
    {
        for (int i = 0; i < cls.size(); i++) {
            ...
            if (cls[i]->count1 == 3 && cls[i]->count2 == 1) {
                ...
                // ARCSim::Faces CAN be added and deleted throughout
                // this loop so we can't just use the CD returned tri
                ARCSim::Face* f0 =
                    get_enclosing_face(mesh, ARCSim::Vec2(xX, yX));
                ARCSim::Vec3 bary = get_barycentric_coords(ARCSim::Vec2(xX, yX), f0);

                bool use_edge = false;
                for (int j = 0; j < 3; j++) {
                    if (bary[j] < 1e-3)
                        use_edge = true;  // TODO:: No magic
                }

                // If this point is on a cloth edge, we should edge
                // split instead
                if (use_edge) {
                    double least = 1.0;
                    int which = -1;
                    ...
                    ARCSim::Edge* e0 = get_opp_edge(f0, f0->v[which]->node);
                    double d;
                    ...
                    ARCSim::Node* node0 = e0->n[0], * node1 = e0->n[1];
                    ARCSim::RemeshOp op = split_edgeForced(e0, d, -1);
                    for (size_t v = 0; v < op.added_verts.size(); v++) {
                        ARCSim::Vert* vertnew = op.added_verts[v];
                        ARCSim::Vert* v0 = adjacent_vert(node0, vertnew),
                    ...
                    ARCSim::RemeshOp op = split_face(f0, bary);
                    for (size_t v = 0; v < op.added_verts.size(); v++) {
                        ARCSim::Vert* vertnew = op.added_verts[v];
                        vertnew->sizing =
            ...
            if (cls[i]->count1 == 2 && cls[i]->count2 == 2) {
                ...
                if (inBoundaryN(boundaries, Vector3d(xX, yX, 0.0),
                    ARCSim::Vec3(e2(0), e2(1), 0.0), boundary))
                    continue;

                // If a previous collisions has already split this edge this
                // get_edge will return NULL and we have to do a little more
                // work to find it
                if (e0 == NULL) {
                    // A barycentric tests largest two values should be the
                    // nodes of the new edge we need to split
                    ARCSim::Vec3 bary = get_barycentric_coords(ARCSim::Vec2(xX, yX), f0);
                    double least = 1.0;
                    int which = -1;
                    for (int j = 0; j < 3; j++) {
                ...
                n->x = e2v(
                    ...
                    ->pos1_);  // We want to offset the node slightly
                               ...
                               // the CD until it moves out on it's own
            }
        }
    }
    ...
    void markPreserve(ARCSim::Mesh& mesh) {
        for (int i = 0; i < mesh.edges.size(); i++) {
            ...
            if (n0->EoL && n1->EoL) {
                ...
                // A corner will have a list of cdEdges, while an edge is garaunteed to
                // only have one cdEdge
                bool match = false;
                if (n0->cornerID >= 0) {
    ...
    bool can_collapseForced(const ARCSim::Edge* edge, int i) {
        for (int s = 0; s < 2; s++) {
            ...
            for (int f = 0; f < (int)vert0->adjf.size(); f++) {
                const ARCSim::Face* face = vert0->adjf[f];
                if (is_in(vert1, face->v)) continue;
                const ARCSim::Vert* vs[3] = { face->v[0], face->v[1], face->v[2] };
                ...
                // for (int e = 0; e < 3; e++)
                //	if (vs[e] != vert1 && edge_metric(vs[NEXT(e)], vs[PREV(e)]) >
                //0.9) { 		return false;
                //	}
            }
        }
    ...
    // For very illconditioned geometry we can potentially have two preserved edges
    // forming a triangle We want to collapse this triangle and just make the non
    // preserved edge the new single preserved edge This triangle was practically a
    // line it was so thin that nothing is really being altered dramatically
    void pass_collapse(ARCSim::RemeshOp op, ARCSim::Node* n) {
        for (int e = 0; e < op.removed_edges.size(); e++) {
            if (op.removed_edges[e]->preserve) {
                if (op.removed_edges[e]->n[0] == n || op.removed_edges[e]->n[1] == n)
                    continue;
                ARCSim::Edge* ep = get_edge(n, op.removed_edges[e]->n[0]);
    ...
    // bool collapse_close(ARCSim::Mesh &mesh)
    //{
    //	for (int i = 0; i < mesh.nodes.size(); i++) {
    //		ARCSim::Node *n0 = mesh.nodes[i];
    //		if (n0->EoL) {
    //			for (int e = 0; e < n0->adje.size(); e++) {
    //				ARCSim::Edge *e0 = n0->adje[e];
    //				ARCSim::Node *n1 = other_node(e0, n0);
    //				if (!n1->EoL && !is_seam_or_boundary(n1) &&
    //edge_length(e0) < thresh) { 					ARCSim::RemeshOp op; 					if (n0 == e0->n[0]) op =
    //collapse_edgeForced(e0, 1); 					else op = collapse_edgeForced(e0, 0); 					if
    //(op.empty()) continue; 					op.done(); 					return true;
    //				}
    //			}
    //		}
    //	}
    ...
    int conformalCount(ARCSim::Face* f) {
        int count = 0;
    ...
    ARCSim::Node* single_eol_from_face(ARCSim::Face* f) {
        for (int vert = 0; vert < 3; vert++) {
    ...
    ARCSim::Edge* single_conformal_edge_from_face(ARCSim::Face* f) {
        for (int e = 0; e < 3; e++) {
    ...
    ARCSim::Edge* single_nonconformal_edge_from_face(ARCSim::Face* f) {
        for (int e = 0; e < 3; e++) {
            if (!f->adje[e]->preserve) return f->adje[e];
        }
        return NULL;
    ...
    bool split_illconditioned_faces(ARCSim::Mesh& mesh) {
        vector<ARCSim::Edge*> bad_edges;
        vector<int> case_pair;
        vector<double> split_point;
        for (int i = 0; i < mesh.faces.size(); i++) {
            ARCSim::Face* f0 = mesh.faces[i];
            int cc = conformalCount(f0);
            if (cc == 1) {
                ARCSim::Node* n0 = single_eol_from_face(f0);
                ARCSim::Edge* e0 = get_opp_edge(f0, n0);
                ...
                if (faceAlti < thresh / 2) {
                    ...
                    for (int ff = 0; ff < n0->verts[0]->adjf.size(); ff++) {
                        ARCSim::Face* f1 = n0->verts[0]->adjf[ff];
                        if (f0 == f1) continue;
                        ARCSim::Edge* e1 = get_opp_edge(f1, n0);
                    ...
                    if (sp < 0.0 || sp > 1.0)
                        continue;  // Just to avoid problems, although if it happens there
                                   // might already be a problem
                    bad_edges.push_back(e0);
                    case_pair.push_back(1);
                    split_point.push_back(sp);
                }
            }
            // TODO:: Cases 2 and 3 may break if large triangles share individual EoL
            // points without preserved edges
            else if (cc == 2) {
                ARCSim::Edge* e0 = single_conformal_edge_from_face(f0);
                if (e0 == NULL)
                    continue;  // If two corner EoL points share a triangle with no
                               // preserved edge
                ...
                ARCSim::Edge* e0 = single_nonconformal_edge_from_face(f0);
                if (face_altitude(e0, f0) < thresh / 2) {
                    bad_edges.push_back(e0);
        ...
        for (size_t e = 0; e < bad_edges.size(); e++) {
            ARCSim::Edge* edge = bad_edges[e];
            if (!edge) continue;
            ARCSim::Node* node0 = edge->n[0], * node1 = edge->n[1];
            ARCSim::RemeshOp op = split_edgeForced(edge, split_point[e], thresh);
            if (op.empty()) {
                exclude++;
                continue;
            }
            ...
            // If we've split a conformal edge, this new node is EoL and must have the
            // appropriate data
            if (case_pair[e] == 2) {
                ARCSim::Node* node = op.added_nodes[0];  // There should only be one?
                node->EoL = true;
                if (node0->EoL_state == ARCSim::Node::IsEOL && node1->EoL_state == ARCSim::Node::IsEOL)
                    node->EoL_state = ARCSim::Node::NewEOLFromSplit;
                else
                    node->EoL_state = ARCSim::Node::NewEOL;
                // Transfer cdEdges from the non corner EoL node
                // This should be safe?
                if (node0->cornerID >= 0)
                    node->cdEdges = node1->cdEdges;
                else
                    node->cdEdges = node0->cdEdges;
            }
    ...
    void cleanup(ARCSim::Mesh& mesh) {
        vector<ARCSim::Face*> active_faces = mesh.faces;
        flip_edges(0, active_faces, 0, 0);
        markPreserve(mesh);
        bool allclear = false;
        while (!allclear) {
            // Iterate until all the bad edges are accounted for
            // If a bad edge exists, but is unsafe to collapse, try in the next
            // iteration where it may become safe, or another operation may take care of
            // it
            while (!allclear) {
                allclear = true;
                while (collapse_nonconformal(mesh, allclear))
                    ;
                markPreserve(mesh);  // Can be removed if the conformal pass doesn't loop
                                     // over edges and check for preserve status
        ...
        markPreserve(mesh);  // Probably doesn't need to be called so much, but wan't
                             // to be safe
    }

    // TODO:: I think there are problems when a box corner reaches the cloth border
    // and the two box edges still move through the cloth

    void preprocess(ARCSim::Mesh& mesh, const MatrixXd& boundaries,
        const vector<shared_ptr<btc::Collision> > cls) {
        markWasEOL(mesh);
    ...
    bool a;
    void preprocessPart(ARCSim::Mesh& mesh, const MatrixXd& boundaries,
        const vector<shared_ptr<btc::Collision> > cls, int& part) {
        if (part == 1) {
            // for (int i = 0; i < mesh.nodes.size(); i++) {
