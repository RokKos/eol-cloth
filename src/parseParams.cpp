...
namespace EOL {
	...
	void load_genset(const Core::Ref<GeneralSettings> general_settings, const string& JSON_FILE)
	{
		Json::Value json;
		Json::Reader reader;
		...
		parse(general_settings->online, json["online"], false);
		parse(general_settings->exportObjs, json["exportObjs"], false);
		parse(general_settings->exportTimings, json["exportTimings"], false);
		parse(general_settings->RESOURCE_DIR, json["RESOURCE_DIR"], string(""));
		if (general_settings->RESOURCE_DIR == "") {
			cout << "Resource directory was not specified." << endl;
			cout << "Do so with the JSON key:" << endl;
			cout << "	\"RESOURCE_DIR\": <path-to-resource-dir>" << endl;
			abort();
		}
		if (general_settings->exportObjs) {
			parse(general_settings->OUTPUT_DIR, json["OUTPUT_DIR"], string(""));
			if (general_settings->OUTPUT_DIR == "") {
		...
		general_settings->RESOURCE_DIR += string("/"); // Just in case

		general_settings->printGenSet();
	}
