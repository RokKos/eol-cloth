...
void nodeBasedF(const ARCSim::Mesh& mesh, VectorXd& f, vector<T>& M_,
                const Vector3d& grav) {
  for (int n = 0; n < mesh.nodes.size(); n++) {
    ...
    for (int j = 0; j < 3; j++) {
      M_.push_back(T(nindex + j, nindex + j,
                     Mxx(j, j)));  // We know it's a diagonal matrix
    }

    if (node->EoL) {
      int nindexX =
          static_cast<int>(mesh.nodes.size()) * 3 + node->EoL_index * 2;
...
void fillxMI(vector<T>& MDK_, vector<T>& M_, const Matrix3d& Kxx,
             const Matrix3d& Mxx, int index, const Vector2d& damping,
             double h) {
...
void fillxxMI(vector<T>& MDK_, vector<T>& M_, const Matrix3d& Kxx,
              const Matrix3d& Mxx, int i0, int i1, const Vector2d& damping,
              double h) {
  Matrix3d MDKxx = Mxx + damping(1) * h * h * Kxx;
  for (int j = 0; j < 3; j++) {
    for (int k = 0; k < 3; k++) {
...
void fillXMI(vector<T>& MDK_, vector<T>& M_, const Matrix2d& KXX,
             const Matrix2d& Mxx, int index, const Vector2d& damping,
             double h) {
...
void fillXXMI(vector<T>& MDK_, vector<T>& M_, const Matrix2d& KXX,
              const Matrix2d& Mxx, int i0, int i1, const Vector2d& damping,
              double h) {
  Matrix2d MDKXX = Mxx + damping(1) * h * h * KXX;
  for (int j = 0; j < 2; j++) {
    for (int k = 0; k < 2; k++) {
...
void fillXxMI(vector<T>& MDK_, vector<T>& M_, MatrixXd& KXx,
              const MatrixXd& Mxx, int i0, int i1, Vector2d& damping,
              double h) {
...
void fillEOLInertia(const ARCSim::Face* face, VectorXd& fi, MatrixXd& Mi) {
  MatrixXd Fa, Fb, Fc;
  ...
  Vector3d fia = fi.segment<3>(0);
  Vector3d fib = fi.segment<3>(3);
  Vector3d fic = fi.segment<3>(6);

  Matrix3d Kiaa = Mi.block<3, 3>(0, 0);
  Matrix3d Kiab = Mi.block<3, 3>(0, 3);
  Matrix3d Kiac = Mi.block<3, 3>(0, 6);
  Matrix3d Kiba = Mi.block<3, 3>(3, 0);
  Matrix3d Kibb = Mi.block<3, 3>(3, 3);
  Matrix3d Kibc = Mi.block<3, 3>(3, 6);
  Matrix3d Kica = Mi.block<3, 3>(6, 0);
  Matrix3d Kicb = Mi.block<3, 3>(6, 3);
  Matrix3d Kicc = Mi.block<3, 3>(6, 6);
  ...
  Mi.block<3, 3>(ja, ja) = Kiaa;
  Mi.block<3, 3>(ja, jb) = Kiab;
  Mi.block<3, 3>(ja, jc) = Kiac;
  /*Mi.block<3, 3>(jb, ja) = Kiba;*/ Mi.block<3, 3>(jb, jb) = Kibb;
  Mi.block<3, 3>(jb, jc) = Kibc;
  /*Mi.block<3, 3>(jc, ja) = Kica; Mi.block<3, 3>(jc, jb) = Kicb;*/ Mi
      .block<3, 3>(jc, jc) = Kicc;
...
void fillEOLMembrane(const ARCSim::Face* face, VectorXd& fm, MatrixXd& Km) {
  MatrixXd Fa, Fb, Fc;
  bool EOLA = face->v[0]->node->EoL;
  bool EOLB = face->v[1]->node->EoL;
  bool EOLC = face->v[2]->node->EoL;
  ...
  Vector3d fma = fm.segment<3>(0);
  Vector3d fmb = fm.segment<3>(3);
  Vector3d fmc = fm.segment<3>(6);

  Matrix3d Kmaa = Km.block<3, 3>(0, 0);
  Matrix3d Kmab = Km.block<3, 3>(0, 3);
  Matrix3d Kmac = Km.block<3, 3>(0, 6);
  Matrix3d Kmba = Km.block<3, 3>(3, 0);
  Matrix3d Kmbb = Km.block<3, 3>(3, 3);
  Matrix3d Kmbc = Km.block<3, 3>(3, 6);
  Matrix3d Kmca = Km.block<3, 3>(6, 0);
  Matrix3d Kmcb = Km.block<3, 3>(6, 3);
  Matrix3d Kmcc = Km.block<3, 3>(6, 6);
  ...
  Km.block<3, 3>(ja, ja) = Kmaa;
  Km.block<3, 3>(ja, jb) = Kmab;
  Km.block<3, 3>(ja, jc) = Kmac;
  /*Km.block<3, 3>(jb, ja) = Kmba;*/ Km.block<3, 3>(jb, jb) = Kmbb;
  Km.block<3, 3>(jb, jc) = Kmbc;
  /*Km.block<3, 3>(jc, ja) = Kmca; Km.block<3, 3>(jc, jb) = Kmcb;*/ Km
      .block<3, 3>(jc, jc) = Kmcc;
...
void faceBasedF(const ARCSim::Mesh& mesh, VectorXd& f, vector<T>& MDK_,
                vector<T>& M_, const Vector3d& grav, double h) {
  for (int i = 0; i < mesh.faces.size(); i++) {
    ARCSim::Face* face = mesh.faces[i];
    ...
    Vector3d txa = v2e(face->v[0]->node->x), txb = v2e(face->v[1]->node->x),
             txc = v2e(face->v[2]->node->x);
    Vector2d tXa = v322e(face->v[0]->u), tXb = v322e(face->v[1]->u),
    ...
    int aindexX = static_cast<int>(mesh.nodes.size()) * 3 +
                  face->v[0]->node->EoL_index * 2;
    int bindexX = static_cast<int>(mesh.nodes.size()) * 3 +
                  face->v[1]->node->EoL_index * 2;
    int cindexX = static_cast<int>(mesh.nodes.size()) * 3 +
                  face->v[2]->node->EoL_index * 2;
    ...
    ComputeMembrane(xa, xb, xc, Xa, Xb, Xc, mat->e, mat->nu, PP, QQ, Wm, fm,
                    Km);
    ...
    if (face->v[0]->node->EoL || face->v[1]->node->EoL ||
      ...
      Mxx = Mie.block<3, 3>(3, 3);
      fillxMI(MDK_, M_, Kxx, Mxx, bindex, damping, h);
      Kxx = Kme.block<3, 3>(6, 6);
      Mxx = Mie.block<3, 3>(6, 6);
      fillxMI(MDK_, M_, Kxx, Mxx, cindex, damping, h);

      Kxx = Kme.block<3, 3>(0, 3);
      Mxx = Mie.block<3, 3>(0, 3);
      fillxxMI(MDK_, M_, Kxx, Mxx, aindex, bindex, damping, h);
      Kxx = Kme.block<3, 3>(0, 6);
      Mxx = Mie.block<3, 3>(0, 6);
      fillxxMI(MDK_, M_, Kxx, Mxx, aindex, cindex, damping, h);
      Kxx = Kme.block<3, 3>(3, 6);
      Mxx = Mie.block<3, 3>(3, 6);
      fillxxMI(MDK_, M_, Kxx, Mxx, bindex, cindex, damping, h);
    }
  }
}
...
void fillEOLBending(const ARCSim::Edge* edge, const ARCSim::Vert* v0,
                    const ARCSim::Vert* v1, const ARCSim::Vert* v2,
                    const ARCSim::Vert* v3, VectorXd& fb, MatrixXd& Kb) {
  ...
  Vector3d fba = fb.segment<3>(0);
  Vector3d fbb = fb.segment<3>(3);
  Vector3d fbc = fb.segment<3>(6);
  Vector3d fbd = fb.segment<3>(9);

  Matrix3d Kbaa = Kb.block<3, 3>(0, 0);
  Matrix3d Kbab = Kb.block<3, 3>(0, 3);
  Matrix3d Kbac = Kb.block<3, 3>(0, 6);
  MatrixXd Kbad = Kb.block<3, 3>(0, 9);
  Matrix3d Kbba = Kb.block<3, 3>(3, 0);
  Matrix3d Kbbb = Kb.block<3, 3>(3, 3);
  Matrix3d Kbbc = Kb.block<3, 3>(3, 6);
  MatrixXd Kbbd = Kb.block<3, 3>(3, 9);
  Matrix3d Kbca = Kb.block<3, 3>(6, 0);
  Matrix3d Kbcb = Kb.block<3, 3>(6, 3);
  Matrix3d Kbcc = Kb.block<3, 3>(6, 6);
  MatrixXd Kbcd = Kb.block<3, 3>(6, 9);
  Matrix3d Kbda = Kb.block<3, 3>(9, 0);
  Matrix3d Kbdb = Kb.block<3, 3>(9, 3);
  Matrix3d Kbdc = Kb.block<3, 3>(9, 6);
  MatrixXd Kbdd = Kb.block<3, 3>(9, 9);
  ...
  Kb.block<3, 3>(ja, ja) = Kbaa;
  Kb.block<3, 3>(ja, jb) = Kbab;
  Kb.block<3, 3>(ja, jc) = Kbac;
  Kb.block<3, 3>(ja, jd) = Kbad;
  /*Kd.block<3, 3>(jb, ja) = Kbba;*/ Kb.block<3, 3>(jb, jb) = Kbbb;
  Kb.block<3, 3>(jb, jc) = Kbbc;
  Kb.block<3, 3>(jb, jd) = Kbbd;
  /*Kd.block<3, 3>(jc, ja) = Kbca; Kb.block<3, 3>(jc, jb) = Kbcb;*/ Kb
      .block<3, 3>(jc, jc) = Kbcc;
  Kb.block<3, 3>(jc, jd) = Kbcd;
  /*Kd.block<3, 3>(jd, ja) = Kbda; Kb.block<3, 3>(jd, jb) = Kbdb; Kb.block<3,
   * 3>(jd, jc) = Kbdc;*/
  Kb.block<3, 3>(jd, jd) = Kbdd;
...
void edgeBasedF(const ARCSim::Mesh& mesh, const Material& mat, VectorXd& f,
                vector<T>& MDK_, double h) {
  for (int e = 0; e < mesh.edges.size(); e++) {
    if (mesh.edges[e]->adjf[0] == NULL || mesh.edges[e]->adjf[1] == NULL) {
      continue;
    }
    ARCSim::Edge* edge = mesh.edges[e];
    ARCSim::Face *f0 = edge->adjf[0], *f1 = edge->adjf[1];
    ARCSim::Node *n0 = edge->n[0], *n1 = edge->n[1];
    ARCSim::Vert *v0 = n0->verts[0], *v1 = n1->verts[0];

    ARCSim::Vert *v2 = get_other_vert(f0, v0, v1),
                 *v3 = get_other_vert(f1, v0, v1);
    ARCSim::Node *n2 = v2->node, *n3 = v3->node;

    double xa[3], xb[3], xc[3], xd[3];
    double Xa[2], Xb[2], Xc[2], Xd[2];

    Vector3d txa = v2e(n0->x), txb = v2e(n1->x), txc = v2e(n2->x),
             txd = v2e(n3->x);
    Vector2d tXa = v322e(v0->u), tXb = v322e(v1->u), tXc = v322e(v2->u),
             tXd = v322e(v3->u);
...
void Forces::fill(const ARCSim::Mesh& mesh, const Material& mat,
                  const Vector3d& grav, double h) {
  ...
  M.resize(mesh.nodes.size() * 3 + mesh.EoL_Count * 2,
           mesh.nodes.size() * 3 + mesh.EoL_Count * 2);
  MDK.resize(mesh.nodes.size() * 3 + mesh.EoL_Count * 2,
             mesh.nodes.size() * 3 + mesh.EoL_Count * 2);
