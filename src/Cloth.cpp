...
namespace EOL {
    ...
    Cloth::Cloth() : fsindex(0) {
        consts = make_shared<Constraints>();
        myForces = make_shared<Forces>();
    }

    void Cloth::build(const Vector2i res, const VectorXd& xX00,
        const VectorXd& xX01, const VectorXd& xX10,
        const VectorXd& xX11) {
        // Break up into world and texture points
        Vector3d x00, x01, x10, x11;
        Vector3d X00 = Vector3d::Zero(), X01 = Vector3d::Zero(),
            X10 = Vector3d::Zero(), X11 = Vector3d::Zero();
        ...
        // Set boundary values
        // In this fixed 4 corner case, the outer boundary is made up of those 4
        // corners
        boundaries.resize(3, 4);
        boundaries.block<3, 1>(0, 0) = X00;
        boundaries.block<3, 1>(0, 1) = X01;
        boundaries.block<3, 1>(0, 2) =
            X11;  // we flip this order to they are sequential of a continueous
                  // boundary path
        ...
        for (int i = 0; i < res(0); ++i) {
            ...
            for (int j = 0; j < res(1); ++j) {
                ...
                Vector3d X = (1 - v) * X0 + v * X1;
                mesh.add(new ARCSim::Vert(e2v(X), ARCSim::Vec3(0)));
                mesh.add(new ARCSim::Node(e2v(x), e2v(x), ARCSim::Vec3(0), 0, 0, false));
        ...
        for (int i = 0; i < res(0) - 1; ++i) {
            for (int j = 0; j < res(1) - 1; ++j) {
                ...
                Vector3d x = (v2e(mesh.nodes[k0]->x) + v2e(mesh.nodes[k0 + 1]->x) +
                    v2e(mesh.nodes[k0 + res(1) + 1]->x) +
                    v2e(mesh.nodes[k0 + res(1)]->x)) /
                    4;
                Vector3d X = (v2e(mesh.verts[k0]->u) + v2e(mesh.verts[k0 + 1]->u) +
                    v2e(mesh.verts[k0 + res(1) + 1]->u) +
                    v2e(mesh.verts[k0 + res(1)]->u)) /
                    4;
                mesh.add(new ARCSim::Vert(e2v(X), ARCSim::Vec3(0)));
                mesh.add(new ARCSim::Node(e2v(x), e2v(x), ARCSim::Vec3(0), 0, 0, false));
                connect(mesh.verts.back(), mesh.nodes.back());
        ...
        for (int i = 0; i < res(0) - 1; i++) {
            for (int j = 0; j < res(1) - 1; j++) {
                ...
                center_index_cnt++;
                vector<ARCSim::Vert*> verts1;
                verts1.push_back(mesh.verts[k0]);
                verts1.push_back(mesh.verts[k0 + 1]);
                verts1.push_back(mesh.verts[kc0]);
                vector<ARCSim::Face*> faces1 = triangulateARC(verts1);
                for (int f = 0; f < faces1.size(); f++) mesh.add(faces1[f]);
                vector<ARCSim::Vert*> verts2;
                verts2.push_back(mesh.verts[k0 + 1]);
                verts2.push_back(mesh.verts[k0 + res(1) + 1]);
                verts2.push_back(mesh.verts[kc0]);
                vector<ARCSim::Face*> faces2 = triangulateARC(verts2);
                for (int f = 0; f < faces2.size(); f++) mesh.add(faces2[f]);
                vector<ARCSim::Vert*> verts3;
                verts3.push_back(mesh.verts[k0 + res(1) + 1]);
                verts3.push_back(mesh.verts[k0 + res(1)]);
                verts3.push_back(mesh.verts[kc0]);
                vector<ARCSim::Face*> faces3 = triangulateARC(verts3);
                for (int f = 0; f < faces3.size(); f++) mesh.add(faces3[f]);
                vector<ARCSim::Vert*> verts4;
                verts4.push_back(mesh.verts[k0 + res(1)]);
                verts4.push_back(mesh.verts[k0]);
                verts4.push_back(mesh.verts[kc0]);
                vector<ARCSim::Face*> faces4 = triangulateARC(verts4);
                for (int f = 0; f < faces4.size(); f++) mesh.add(faces4[f]);
            }
        }
    ...
    void Cloth::updatePosNor() {
        for (int i = 0; i < mesh.nodes.size(); i++) {
            ARCSim::Vec3 xm = mesh.nodes[i]->x;
            ARCSim::Vec3 Xm = mesh.nodes[i]->verts[0]->u;
            ARCSim::Vec3 nm = mesh.nodes[i]->n;
            ARCSim::Vec3 tm = mesh.nodes[i]->verts[0]->u;
            posBuf[3 * i + 0] = xm[0];
            posBuf[3 * i + 1] = xm[1];
            posBuf[3 * i + 2] = xm[2];
            norBuf[3 * i + 0] = nm[0];
    ...
    void Cloth::velocityTransfer() {
        ...
        for (int n = 0; n < mesh.nodes.size(); n++) {
            ...
            // Search through the unremeshed mesh to see if this node existed before, or
            // was just introduced it this step
            ...
            for (int j = 0; j < last_mesh.nodes.size(); j++) {
                double dist =
                    unsigned_vv_distance(vert->u, last_mesh.nodes[j]->verts[0]->u);
                if (unsigned_vv_distance(vert->u, last_mesh.nodes[j]->verts[0]->u) <
                    how_close) {
                    how_close = dist;
                    closest = j;
                    found = true;
            ...
            if (found) {
                // If the node was EOL but is no longer, we have to transfer it's EOL
                // components back into fully LAG
                if (node->EoL_state == ARCSim::Node::WasEOL) {
                    node->EoL_state = ARCSim::Node::IsLAG;
                ...
                // If it already existed, just pull it's old information
                    ...
                    // TODO:: The node will have the data if it gets here
                    // correct?
                    v(3 * n) = node->v[0];
                    v(3 * n + 1) = node->v[1];
                    v(3 * n + 2) = node->v[2];
            ...
            // If the node did not exist We have to do one of four things
                ...
                if (!node->EoL) {
                    ARCSim::Face* old_face =
                        get_enclosing_face(last_mesh, ARCSim::Vec2(vert->u[0], vert->u[1]));
                    ARCSim::Vec3 bary = get_barycentric_coords(
                        ARCSim::Vec2(vert->u[0], vert->u[1]), old_face);
                    Vector3d vwA = v2e(old_face->v[0]->node->v);
                    Vector3d vwB = v2e(old_face->v[1]->node->v);
                    Vector3d vwC = v2e(old_face->v[2]->node->v);
                    // If any of its surrounding points are EOL, the Eulerian
                    // component velocity component must be taken into account
                    if (old_face->v[0]->node->EoL)
                        vwA += -deform_grad(old_face) * v322e(old_face->v[0]->v);
                    if (old_face->v[1]->node->EoL)
                        vwB += -deform_grad(old_face) * v322e(old_face->v[1]->v);
                    if (old_face->v[2]->node->EoL)
                        vwC += -deform_grad(old_face) * v322e(old_face->v[2]->v);
                    ...
                    // If the new EOL was the result of a conserved edge split,
                    // we average the components for efficiency For this to
                    // happen neither adjacent nodes can be NewEOLs, otherwise
                    // the averaging would be wrong
                    if (node->EoL_state == ARCSim::Node::NewEOLFromSplit) {
                        Vector3d newvL = Vector3d::Zero();
                        Vector3d newvE = Vector3d::Zero();
                        for (int e = 0; e < node->adje.size(); e++) {
                            if (node->adje[e]->preserve) {
                                ARCSim::Node* adjn = other_node(node->adje[e], node);
                                newvL += v2e(adjn->v);
                    ...
                    // If this is brand new EOL with no previuosly known information,
                    // we calculate its world velocity and distribute it
                    else {
                        ARCSim::Face* old_face = get_enclosing_face(
                            last_mesh, ARCSim::Vec2(vert->u[0], vert->u[1]));
                        Matrix2d ftf =
                            deform_grad(old_face).transpose() * deform_grad(old_face);
                        Vector2d dtv = -deform_grad(old_face).transpose() * v2e(node->v);

                        // We set up a simple KKT system with the purpose of putting as much
                        // world velocity as possible into the Eulerian component
                        ...
                        vert->v = ARCSim::Vec3(newvE(0), newvE(1), 0.0);
                        v(mesh.nodes.size() * 3 + node->EoL_index * 2) = newvE(0);
                        v(mesh.nodes.size() * 3 + node->EoL_index * 2 + 1) = newvE(1);

                        // We use the deformation gradient to extract the lagrangian component
                        Vector3d newvL =
                            v2e(node->v) + -deform_grad(old_face) * newvE.segment<2>(0);
                        node->v = e2v(newvL);
                        v(3 * n) = newvL(0);
                        v(3 * n + 1) = newvL(1);
                        v(3 * n + 2) = newvL(2);
    ...
    void Cloth::solve(shared_ptr<GeneralizedSolver> gs, double h) {
        ...
        generateMatlab(myForces->M, myForces->MDK, b, consts->Aeq, consts->beq,
            consts->Aineq, consts->bineq, v);
        bool success = gs->velocitySolve(consts->hasFixed, consts->hasCollisions,
            myForces->MDK, b, consts->Aeq, consts->beq,
            consts->Aineq, consts->bineq, v);
        vec_to_file(v, "v_solved", "solver.m", false);
    }

    void Cloth::step(shared_ptr<GeneralizedSolver> gs, shared_ptr<Obstacles> obs,
        const Vector3d& grav, double h, const bool& REMESHon,
        const bool& online) {
        ...
        for (int i = 0; i < mesh.faces.size(); i++) {
            faces2.col(i) = Vector3i(mesh.faces[i]->v[0]->node->index,
                mesh.faces[i]->v[1]->node->index,
                mesh.faces[i]->v[2]->node->index);
        }
        VectorXi vvv(3);
        vvv << 1, 1, 1;
        ...
        for (int n = 0; n < mesh.nodes.size(); n++) {
            ...
            if (node->EoL) {
                ...
                // if (mesh.nodes[i]->verts[0]->u[0] != Xmin &&
                // mesh.nodes[i]->verts[0]->u[0] != Xmax) mesh.nodes[i]->verts[0]->u[0] =
                // mesh.nodes[i]->verts[0]->u[0] + h * mesh.nodes[i]->verts[0]->v[0]; if
                // (mesh.nodes[i]->verts[0]->u[1] != Ymin && mesh.nodes[i]->verts[0]->u[1]
                // != Ymax) mesh.nodes[i]->verts[0]->u[1] = mesh.nodes[i]->verts[0]->u[1]
                // + h * mesh.nodes[i]->verts[0]->v[1];
    ...
    void Cloth::draw(shared_ptr<Rendering::MatrixStack> MV,
        const shared_ptr<Rendering::Program> p) const {
        // Draw mesh
    ...
    void Cloth::drawSimple(shared_ptr<Rendering::MatrixStack> MV,
        const shared_ptr<Rendering::Program> p) const {
        for (int i = 0; i < mesh.nodes.size(); i++) {
            if (!mesh.nodes[i]->EoL) {
    ...
    // Export
    int Cloth::getBrenderCount() const { return 2; }
    ...
    void Cloth::exportBrender(vector<shared_ptr<ofstream> > outfiles) const {
        ...
        for (int i = 0; i < mesh.nodes.size(); i++) {
            char vert[50];
            sprintf(vert, "v %f %f %d\n", mesh.nodes[i]->verts[0]->u[0],
                mesh.nodes[i]->verts[0]->u[1], 0);
            outfile << vert;
        }
        // texture coordinates
        ...
        for (int i = 0; i < eleBuf.size(); i = i + 3) {
            ...
            sprintf(face, "f %i/%i/%i %i/%i/%i %i/%i/%i\n", eleBuf[i] + 1,
                eleBuf[i] + 1, eleBuf[i] + 1, eleBuf[i + 1] + 1, eleBuf[i + 1] + 1,
                eleBuf[i + 1] + 1, eleBuf[i + 2] + 1, eleBuf[i + 2] + 1,
                eleBuf[i + 2] + 1);
            outfile << face;
        }
